...
def echo_callback(update: Update, context: CallbackContext):
    context.bot.send_message(chat_id=update.effective_chat.id, text=simple.echo(update.message.text))
...
def random_important_fact_callback(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(
        ...
        reply_to_message_id=update.message.message_id if chat_type == 'group' else None,
    )
